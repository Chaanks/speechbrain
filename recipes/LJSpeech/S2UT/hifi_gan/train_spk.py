...
class HifiGanBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        y_g_hat, (log_dur_pred, log_dur) = self.modules.generator(x, spk, stage=stage)
    ...
    def run_inference_sample(self, name):
        ...
        with torch.no_grad():
            ...
            sig_out = inference_generator.inference(x, spk)
...
def dataio_prepare(hparams):
    ...
    speaker_manager = EmbeddingManager(
        hparams["embedding_path"],
        hparams["lang"],
        hparams["splits"],
    )
    ...
    def spk_pipeline(utt_id):
        spk_emb = speaker_manager.get_embedding_by_clip(utt_id)
    ...
    for dataset in hparams["splits"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            output_keys=["id", "sig", "feats", "spk_emb"],
        )
