"""Transformer for S2U in the SpeechBrain sytle.
...
class TransformerS2U(TransformerInterface):
    """This is an implementation of transformer model for S2U.
    ...
    forward_decoder_only(self, src, tgt, pad_idx=0)
        ...
        if self.attention_type == "RelPosMHAXL":
            ...
            tgt = tgt + self.positional_encoding_decoder(tgt)
            # FIXME we use pos embs also on enc output
            src = src + self.positional_encoding_decoder(
                src
            )
            pos_embs_encoder = None  # self.positional_encoding(src)
            pos_embs_target = None
...
class EncoderWrapper(nn.Module):
    """This is a wrapper of any S2U transformer encoder. By default, the
    TransformerS2U .forward() function encodes and decodes. With this wrapper
    the .forward() function becomes .encode() only.

    Important: The TransformerS2U class must contain a .encode() function.
